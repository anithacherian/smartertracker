...
def nurse_dashboard(request):
    if request.user.userprofile.role != 'nurse':
        messages.error(request, "Access denied. Only nurse can access this page.")
        return redirect_dashboard(request.user)
...
def receptionist_dashboard(request):
    if request.user.userprofile.role != 'receptionist':
        messages.error(request, "Access denied. Only receptionist can access this page.")
        return redirect_dashboard(request.user)
...
def add_patient(request):
    if request.user.userprofile.role != 'receptionist':
        messages.error(request,'Access denied. Only Receptionist can add patients.')
        return redirect('receptionist_dashboard')
    if request.method == 'POST':
...
def edit_patient(request,pk):
    if request.user.userprofile.role != 'receptionist':
        messages.error(request,'Access denied. Only Receptionist can update patients.')
        return redirect('receptionist_dashboard')
...
def delete_patient(request,pk):
    if request.user.userprofile.role != 'receptionist':
        messages.error(request,'Access denied. Only Receptionist can delete patients.')
        return redirect('receptionist_dashboard')
...
def update_patient_status(request,pk):
    if request.user.userprofile.role != 'doctor':
        messages.error(request,'Access denied. Only Doctor can update patient status.')
        return redirect('receptionist_dashboard')
...
def edit_resource(request,pk):
    if request.user.userprofile.role != 'admin':
        messages.error(request,'Access denied. Only Administrator can edit resources.')
        return redirect('admin_dashboard')
...
def doctor_list(request):
    if request.user.userprofile.role != 'admin':
        messages.error(request,'Access denied. Only Adminisrator  can view doctors.')
        return redirect('receptionist_dashboard')
